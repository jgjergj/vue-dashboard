...
<script lang="ts">
	...
	export default class Team extends Vue {
		...
		async created(): Promise<void> {
			[this.states, this.leagues, this.sports] = await Promise.all([
		...
		mounted() {
			this.teamsCient.getAll().then((res) => {
				this.items = res;
			});
		}
		...
		deleteItemConfirm() {
			...
			this.teamsCient.delete(this.editedItem.id);
			// this.editedIndex
		...
		save() {
			const editItem: any = {
				...
				leagueId: this.editedItem.league.id,
				stateId: this.editedItem.state.id,
			};

			if (this.editedIndex > -1) {
				this.teamsCient.update(this.editedItem.id, editItem);
				Object.assign(this.items[this.editedIndex], this.editedItem);
			} else {
				this.teamsCient.create(editItem);
